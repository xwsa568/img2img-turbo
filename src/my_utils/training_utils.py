...
def parse_args_unpaired_training():
   ...
   """
    ...
    parser.add_argument("--lambda_cycle", default=1, type=float, help="Weight for the second-order DINO Gram cycle loss.")
    parser.add_argument("--lambda_cycle_lpips", default=10.0, type=float, help="Deprecated; cycle consistency now uses second-order DINO Gram loss.")
    parser.add_argument("--lambda_idt_lpips", default=1.0, type=float)
    parser.add_argument("--cycle_gram_model_name", default="dinov2_vitb14", type=str)
    parser.add_argument("--cycle_gram_resize", default=256, type=int, help="Resize used before DINOv2 Gram cycle loss. Use <=0 to keep input size.")
    parser.add_argument("--cycle_gram_tokens", default=0, type=int, help="Shared DINO patch-token subsample for Gram cycle loss. 0 uses all tokens.")
    parser.add_argument("--cycle_gram_tau", default=0.1, type=float, help="Softmax temperature for second-order Gram KL.")
    parser.add_argument("--cycle_gram_eps", default=1e-6, type=float)
    parser.add_argument("--cycle_gram_remove_diag", action="store_true")
    parser.add_argument("--cycle_gram_no_l2norm", action="store_true")

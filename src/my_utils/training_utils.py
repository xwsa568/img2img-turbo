...
def parse_args_unpaired_training():
   ...
   """
    ...
    parser.add_argument("--lambda_cycle", default=1, type=float, help="Weight for fake-vs-source-image second-order DINO relation consistency.")
    parser.add_argument("--lambda_cycle_lpips", default=10.0, type=float, help="Deprecated; unused by DINO relation consistency.")
    parser.add_argument("--lambda_idt_lpips", default=1.0, type=float)
    parser.add_argument("--dino_rel_model_name", "--cycle_gram_model_name", dest="dino_rel_model_name", default="dinov2_vitb14", type=str)
    parser.add_argument("--dino_rel_resize", "--cycle_gram_resize", dest="dino_rel_resize", default=256, type=int, help="Resize used before DINOv2 relation loss. Use <=0 to keep input size.")
    parser.add_argument("--dino_rel_resolutions", default=None, type=str, help="Comma-separated multi-scale DINO resolutions for relation loss. Defaults to --dino_rel_resize.")
    parser.add_argument("--dino_rel_tokens", "--cycle_gram_tokens", dest="dino_rel_tokens", default=0, type=int, help="Shared DINO patch-token subsample for relation loss. 0 uses all tokens.")
    parser.add_argument("--dino_rel_tau", "--cycle_gram_tau", dest="dino_rel_tau", default=0.1, type=float, help="Softmax temperature for second-order Gram KL.")
    parser.add_argument("--dino_rel_eps", "--cycle_gram_eps", dest="dino_rel_eps", default=1e-6, type=float)
    parser.add_argument("--dino_rel_remove_diag", "--cycle_gram_remove_diag", dest="dino_rel_remove_diag", action="store_true")
    parser.add_argument("--dino_rel_no_l2norm", "--cycle_gram_no_l2norm", dest="dino_rel_no_l2norm", action="store_true")
